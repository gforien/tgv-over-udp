...
public class Worker2 extends Serveur {
    ...
    public Worker2(int port, String ip, int debugLevel, int bufferSize, int timeout) throws IOException {
        super(port, ip);
    ...
    public void run() {
        try {
            ...
            while (nBytesLus != -1) {
                ...
                log(2, "paquet "+seq+" stocké");
                seq++;
            }
            seq--;
            log(2, seq+" paquets lus");
...
//            int nBytesLus = 0;
            ...
            while(seq != seqMax || dernierAckRecu != (seq-1)) {

                //  (1) Envoi
                while (seq != seqMax && dernierAckRecu+cwnd >= seq) {
